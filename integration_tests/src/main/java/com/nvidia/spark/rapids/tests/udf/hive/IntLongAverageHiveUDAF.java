...
public class IntLongAverageHiveUDAF extends AbstractGenericUDAFResolver implements RapidsUDAF {
  ...
  @Override
  public ColumnVector getResult(int numRows, ColumnVector[] args, DataType outType) {
    ...
    try (ColumnVector sumCol = args[0];
      ...
      if (args.length != 2) {
        throw new IllegalArgumentException("Expect two column for 'getResult'.");
      }
  ...
  public RapidsUDAFGroupByAggregation updateAggregation() {
    return new RapidsSimpleGroupByAggregation() {
      @Override
      public ColumnVector[] preStep(int numRows, ColumnVector[] args) {
        try (ColumnVector inputInt = args[0]) {
          if (args.length != 1) {
            throw new IllegalArgumentException("Expect only one column for 'preStep'.");
          }
      ...
      @Override
      public ColumnVector[] postStep(int numRows, ColumnVector[] aggregatedData) {
    ...
    return new RapidsSimpleGroupByAggregation() {
      ...
      public Scalar[] reduce(int numRows, ColumnVector[] preStepData) {
        if (preStepData.length != 2) {
          throw new IllegalArgumentException("Expect two column for merge 'reduce'.");
        }
      ...
      public GroupByAggregationOnColumn[] aggregate(int[] inputIndices) {
        if (inputIndices.length != 2) {
          throw new IllegalArgumentException("Expect two column for merge 'aggregate'.");
        }
        return new GroupByAggregationOnColumn[]{
