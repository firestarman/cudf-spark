...
object PreProjectSplitIterator {
  ...
  private def getColumnSize(col: ColumnView, colType: DataType, nullable: Boolean,
    ...
    } else if (colType.isInstanceOf[StringType] || colType.isInstanceOf[BinaryType]) {
      // two special cases for data size
      withResource(col.getChildColumnView(0)) { dataView =>
        assert(dataView.getData != null, s"data view should not be null for $colType")
