...
import org.apache.spark.sql.rapids.aggregate.GpuUDAFUtils
        ...
        (a, conf, p, r) => new TypedImperativeAggExprMeta[HiveUDAFFunction](a, conf, p, r) {
          ...
          override def aggBufferAttribute: AttributeReference = {
            opRapidsFunc.map { rapidsUDAF =>
              GpuUDAFUtils.structAttrFromTypes(expr.name, rapidsUDAF.bufferTypes())
            }.getOrElse( // opRapidsFunc is None, so it will fallback to CPU.
