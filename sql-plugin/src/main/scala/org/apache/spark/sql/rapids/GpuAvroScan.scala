...
case class GpuAvroMultiFilePartitionReaderFactory(
    ...
    queryUsesInputFile: Boolean) extends  {
...
trait GpuAvroReaderBase extends Arm with Logging {
  ...
  protected def sendToGpu(
      ...
      metrics: Map[String, GpuMetric]): Option[ColumnarBatch] = {
    withResource(hostBuf) { _ =>
      if (bufSize == 0) {
        None
      } else {
        // Dump buffer for debugging when required
        dumpDataToFile(hostBuf, bufSize, splits, debugDumpPrefix, Some("avro"))

        val readOpts = CudfAvroOptions.builder()
          .includeColumn(readSchema.fieldNames.toSeq: _*)
          .build()
        ...
        withResource(table) { t =>
          ...
          // convert to batch
          GpuColumnVector.from(t, GpuColumnVector.extractTypes(readSchema))
        }
      } // end of else
    }
  }
  ...
  protected def readPartFile(
      ...
      partFilePath: Path,
      metrics: Map[String, GpuMetric]): (HostMemoryBuffer, Long) = {
      ...
      withResource(partFilePath.getFileSystem(conf).open(partFilePath)) { in =>
        closeOnExcept(HostMemoryBuffer.allocate(estOutSize)) { hmb =>
          withResource(new HostMemoryOutputStream(hmb)) { out =>
            copyBlocks(blockMeta, in, out, metrics)
      ...
      if (readDataSchema.isEmpty) {
        ...
        if (currentChunkedBlocks.isEmpty) {
          ...
          val (dataBuffer, dataSize) = readPartFile(
            AvroBlockMeta(blockMeta.header, currentChunkedBlocks), partFilePath, metrics)
          sendToGpu(dataBuffer, dataSize, readDataSchema, Array(partFile), metrics)
        }
      }
    }
  }
...
class GpuMultiFileCloudAvroPartitionReader(
  ...
  override def readBatch(fileBufsAndMeta: HostMemoryBuffersWithMetaDataBase):
    ...
    case buffer: AvroHostMemoryBuffersWithMetaData =>
      ...
      val optBatch = if (dataBuf == null) {
        ...
        val maybeBatch = sendToGpu(dataBuf, dataSize, readDataSchema, files, metrics)
      ...
      optBatch
    case t =>
      throw new RuntimeException("Unknown avro buffer type '${t.getClass.getSimepleName}'")
  }
        ...
        if (readDataSchema.isEmpty) {
          ...
          while (blockChunkIter.hasNext) {
            ...
            hostBuffers += readPartFile(
              AvroBlockMeta(blockMeta.header, blocksToRead), partFilePath)
          }
          val bytesRead = fileSystemBytesRead() - startingBytesRead
 ...
 */
private case class AvroBlockMeta(header: Header, blocks: Seq[BlockInfo])
